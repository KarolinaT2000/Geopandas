...
gdf = gpd.read_file('PD_STAT_GRID_CELL_2011.shp')
gdf = gdf.to_crs("EPSG:4326")
gdf.plot("MALE_0_14",legend=True)
gdf['centroid']=gdf.centroid
#%%D
import shapely
xmin, ymin,xmax,ymax = [13,48,25,56]
...
ax.set_axis_on()
plt.axis("equal");
plt.title('liczba ludnosci męskiej w przedziałach 0-14  lat w siatce')
...
gdf = gpd.read_file('PD_STAT_GRID_CELL_2011.shp')
gdf = gdf.to_crs("EPSG:4326")
gdf.plot("FEM_RATIO",legend=True)
gdf['centroid']=gdf.centroid
#%%F
import shapely
xmin, ymin,xmax,ymax = [13,48,25,56]
...
ax.set_axis_on()
plt.axis("equal");
plt.title('ratio liczby ludnosci do powierzchni dla danego województwa w siatce')
